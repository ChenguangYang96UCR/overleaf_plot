...
Click a node to open the node toolbar. The toolbar can change its text, font family, font size, shape, fill color, text color, border color, and border width. Supported fonts are serif, sans serif, and monospace. Supported shapes are rectangle, rounded rectangle, ellipse, and diamond. Double-click a node to focus and select its text for editing. Press **Enter** or move focus away to apply the change; press **Escape** to cancel it.

Click an edge to open the edge toolbar. Arrow direction can be forward, reverse, bidirectional, or disabled. The connection can be straight, horizontal then vertical (`-|`), or vertical then horizontal (`|-`).
...
8. Change its font to **Sans serif**, font size to `14 pt`, text color to **Blue**, and border width to `1.2 pt`.
9. Select the edge from **Successful?** to **Done**, set its arrow to **Both directions**, and use **Horizontal then vertical**.
10. Click **Copy LaTeX**, replace the original `tikzpicture` block in Overleaf, and compile again.
...
- The selected edge is bidirectional and uses an orthogonal connection.
